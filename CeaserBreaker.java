...
public class CeaserBreaker
{
	...
	public CeaserBreaker(Key key)
	{	
		setKeyType(key);
		this.decryptedStr=decrypt(getContent());
	}
	public CeaserBreaker(String encryptedMsg,Key key)
	{
		setKeyType(key);
		this.decryptedStr=decrypt(encryptedMsg);
	}	
	public CeaserBreaker(File file,Key key)
	{
		setKeyType(key);
		this.decryptedStr=decrypt(getContent(file));
	}
	public CeaserBreaker(int firstKey,int secondKey)
	{
		this.key=Key.MULTIPLE;
		this.decryptedStr=decrypt(getContent(),firstKey,secondKey);
	}
	public CeaserBreaker(String encryptedMsg,int firstKey,int secondKey)
	{
		this.key=Key.MULTIPLE;
		this.decryptedStr=decrypt(encryptedMsg,firstKey,secondKey);
	}
	public CeaserBreaker(File file,int firstKey,int secondKey)
	{
		this.key=Key.MULTIPLE;
	...
	private int getKey(String encryptedMsg)
	{
		...
		if(idxFreq!=-1)
		{
			if(idxFreq<alphabit.indexOf('e'))
				shift=26-(alphabit.indexOf('e')-idxFreq);
			else
				shift=idxFreq-alphabit.indexOf('e');
		}
		return shift;
	}
